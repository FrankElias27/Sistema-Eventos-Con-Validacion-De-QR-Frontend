...
@Component({
  selector: 'app-modal-mail-configuration',
  templateUrl: './modal-mail-configuration.component.html',
  styleUrls: ['./modal-mail-configuration.component.css']
})
export class ModalMailConfigurationComponent implements OnInit {
  ...
  closeModal() {
    this.ModalService.cerrarModalMail();
  }
  ...
  deleteLogs(event: MouseEvent) {
    event.stopPropagation();
    event.preventDefault();

    Swal.fire({
      title: '¿Estás seguro?',
      text: '¿Deseas eliminar todos los logs de email para este usuario?',
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        this.EmaillogService.deleteEmailLogsByUsuarioId(this.usuarioId).subscribe({
          next: (response) => {
            Swal.fire({
              title: 'Eliminado!',
              text: response,
